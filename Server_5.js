...
const signer = new ethers.Wallet("0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a", provider);
...
const save = async (block) => {
    const latestNonce = await provider.getTransactionCount(signer.address); // Get latest nonce
    const transaction = await contract.interface.encodeFunctionData("save", [block]);
...
const distributeTokens = async (hopArray) => {
    const latestNonce = await provider.getTransactionCount(signer.address)
    const transaction = await contract.interface.encodeFunctionData("distributeTokens", [hopArray]);
    const signedTransaction = await signer.signTransaction({
        to: contract.address,
        nonce: latestNonce + 1,
        data: transaction,
